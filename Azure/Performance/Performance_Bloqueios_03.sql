...
If(Object_ID(N'tempdb..#tmpHeadBlockers') IS NOT NULL)
	DROP TABLE #tmpHeadBlockers

If(Object_ID(N'tempdb..#tmpHeadBlockerDepth') IS NOT NULL)
	DROP TABLE #tmpHeadBlockerDepth 

DECLARE @WaitThreshold INT = 1 * 1000 -- Segundos
...
SELECT
	...
	WHERE
		...
		(
			...
			AND R.wait_time > @WaitThreshold
			AND (S.is_user_process <> 0 OR R.[status] COLLATE Latin1_General_BIN NOT IN('background', 'sleeping'))
		)
		-- Cabeçalho dos Bloqueios
		OR
		(
			S.session_id IN
			(
				SELECT S1.session_id
					FROM sys.dm_exec_sessions S1
						JOIN sys.dm_exec_requests R1 ON R1.blocking_session_id = S1.session_id
						LEFT OUTER JOIN sys.dm_exec_requests R2 ON R2.session_id = S1.session_id
					WHERE R2.blocking_session_id = 0 OR R2.blocking_session_id IS NULL
			)
		)
		AND S.session_id >= 50

-- Procura os tipos ou níveis de bloqueios
;WITH BlockingLevels(SessionID, BlockingSessionID, BlockingLevel, HeadBlocker) AS
(
...
-- Esta query pode ser recolhida na consulta acima. Arqui é dividido para evitar um consumo alto de memória.
SELECT
		TB.SessionID, 
