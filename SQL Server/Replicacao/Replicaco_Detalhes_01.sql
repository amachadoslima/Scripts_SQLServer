...
IF OBJECT_ID('tempdb..#ReplStats') IS NOT NULL  
...
INSERT INTO #ReplStats 
	...
	FROM distribution..MSdistribution_status ds 
		JOIN distribution..MSdistribution_agents da ON da.id = ds.agent_id                          
		JOIN distribution..MSArticles ma ON ma.publisher_id = da.publisher_id AND ma.article_id = ds.article_id
		JOIN distribution..MSreplication_monitordata md ON md.job_id = da.job_id
		JOIN distribution..MSdistribution_history dh ON dh.agent_id = md.agent_id AND md.agent_type = 3
		JOIN master.sys.servers  s ON s.server_id = da.subscriber_id 
