...
begin try
	...
	declare spid cursor for 
		select s.session_id, db_name(r.database_id), s.login_name, r.start_time, s.host_process_id, s.[status], [host_name]
			from sys.dm_exec_sessions s
				join sys.dm_exec_requests r on s.session_id = r.session_id
			where s.session_id > 50	and s.session_id <> @@spid
				
	open spid
	fetch next from spid into @spid, @db, @loginname, @iniexec, @pid, @status, @hostname

		while(@@fetch_status = 0)
		begin
		
			insert into @osql (spid, db, login_name, ini_exec, [status], [host_name])--, pid)
				select @spid, @db, @loginname, @iniexec, @status, case when isnull(@hostname, '') <> '' then @hostname else null end--, @pid
