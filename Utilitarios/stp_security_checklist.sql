...
Begin
    ...
    --DECLARE @Language VARCHAR(2) = 'pt'
	--DECLARE @Heavy_Operations BIT = 1
	...
	(
        IDVerificacao INT NOT NULL PRIMARY KEY,
        DSCategoria VARCHAR(100) NULL,
        DSTitulo VARCHAR(100) NULL,
        DSResultado VARCHAR(100) NULL,
        DSDescricao VARCHAR(MAX) NULL,
        DSVerificacao VARCHAR(MAX) NULL,
        DSSugestao VARCHAR(MAX) NULL,
        DSReferencia VARCHAR(500) NULL,
        DSDetalhes XML NULL
    )
    

    DECLARE @Resultado XML
    DECLARE @ResultadoString VARCHAR(MAX)
	DECLARE @Versao INT
	DECLARE @Quantidade INT
	DECLARE @Data DATETIME
	DECLARE @IsAmazonRDS BIT = 
		(
			CASE 
    ...
    Set @Versao =
		(
			CASE LEFT(CONVERT(VARCHAR, SERVERPROPERTY('ProductVersion')), 2)
				WHEN '8.' THEN 2000
				WHEN '9.' THEN 2005
				WHEN '10' THEN 2008
				WHEN '11' THEN 2012
    ...
    Begin
        ...
        UPDATE #Resultado
        Set DSDescricao = 'ComputerNamePhysicalNetBIOS: ' + COALESCE(CAST(SERVERPROPERTY('ComputerNamePhysicalNetBIOS') AS VARCHAR(500)), '') + 
		                  ' | Nome do servidor: ' + COALESCE(CAST(SERVERPROPERTY('MachineName') AS VARCHAR(500)), '') + 
						  ' | Instância: ' + COALESCE(ISNULL(CAST(SERVERPROPERTY('InstanceName') AS VARCHAR(500)), 'MSSQLSERVER'), '') + ' 
						  | Porta: ' + COALESCE(CAST(@PortaUtilizada AS VARCHAR(20)), '1433')
        WHERE IDVerificacao = -6

        UPDATE #Resultado
        Set DSDescricao = CONVERT(VARCHAR(10), @Data, 103) + ' ' + CONVERT(VARCHAR(10), @Data, 108) + ' (' + CONVERT(VARCHAR(10), DATEDIFF(DAY, @Data, GETDATE())) + ' dias atrás)'
        WHERE IDVerificacao = -5
    ...
    Begin
        ...
        UPDATE #Resultado
        Set DSDescricao = 'ComputerNamePhysicalNetBIOS: ' + COALESCE(CAST(SERVERPROPERTY('ComputerNamePhysicalNetBIOS') AS VARCHAR(500)), '') + 
		                  ' | Server name: ' + COALESCE(CAST(SERVERPROPERTY('MachineName') AS VARCHAR(500)), '') + 
						  ' | Instance name: ' + COALESCE(ISNULL(CAST(SERVERPROPERTY('InstanceName') AS VARCHAR(500)), 'MSSQLSERVER'), '') + 
						  ' | Port: ' + COALESCE(CAST(@PortaUtilizada AS VARCHAR(20)), '1433')
        WHERE IDVerificacao = -6

        UPDATE #Resultado
        Set DSDescricao = CONVERT(VARCHAR(10), @Data, 103) + ' ' + CONVERT(VARCHAR(10), @Data, 108) + ' (' + CONVERT(VARCHAR(10), DATEDIFF(DAY, @Data, GETDATE())) + ' days ago)'
        WHERE IDVerificacao = -5
    ...
    Begin
        ...
        DECLARE @Instancias VARCHAR(MAX)
		DECLARE @Qt_Instancias INT = (SELECT COUNT(*) FROM @GetInstances)
        ...
        Begin
            ...
            UPDATE #Resultado
            Set DSDescricao = CAST(@Qt_Instancias AS VARCHAR(10)) + ' instâncias instaladas: ' + @Instancias
            WHERE IDVerificacao = -3
        ...
        Begin
            ...
            UPDATE #Resultado
            Set DSDescricao = CAST(@Qt_Instancias AS VARCHAR(10)) + ' SQL Server instances on this server: ' + @Instancias
            WHERE IDVerificacao = -3
    ...
    Begin
        ...
        Set 
			...
			(
				CASE 
					WHEN @Resultado IS NULL THEN 'OK' 
					Else 'Possível problema encontrado' 
				END
			),
            DSDetalhes = @Resultado
        WHERE IDVerificacao = 1
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possível problema encontrado' END),
            DSDetalhes = @Resultado
        WHERE 
            IDVerificacao = 2
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Nivel_Auditoria>', 'AuditLevel>'), '0 - Nenhum', '0 - None'), '1 - Apenas logins com sucesso', '1 - Successful logins only')
        WHERE IDVerificacao = 2
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Nivel_Auditoria_Login>', 'Login_Audit_Level>'), '0 - Autenticação Windows e SQL Server', '0 - Windows and SQL Server Authentication')
        WHERE 
            IDVerificacao = 3
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Configuracao>', 'Configuration>')
        WHERE IDVerificacao = 4
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possível problema encontrado (pelo menos ' + CONVERT(VARCHAR(10), @Quantidade) + ' dias sem atualização)' END),
            DSDetalhes = @Resultado
        WHERE 
            IDVerificacao = 6
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found (at least ' + CONVERT(VARCHAR(10), @Quantidade) + ' days without updates)' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Inicialização>', 'Startup>')
        WHERE 
            IDVerificacao = 6
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Configuracao_Server_Triggers>', 'Server_Triggers>')
        WHERE 
            IDVerificacao = 15
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Configuracao_Trace_Habilitado>', 'Trace_Enabled>')
        WHERE 
            IDVerificacao = 16
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Configuracao_XE_Habilitado>', 'Extended_Event_Enabled>')
        WHERE 
            IDVerificacao = 17
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Configuracao_AC_Habilitado>', 'Contained_AC_Enabled>')
            WHERE 
                IDVerificacao = 18
    ...
    Else Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Language = 'pt' THEN 'Não suportado' Else 'Not supported' END)
        WHERE 
            IDVerificacao = 18
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Configuracao_EL_Habilitado>', 'Error_Log>')
        WHERE 
            IDVerificacao = 19
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Configuracao_SQLAuth_Habilitado>', 'Error_Log>')
            WHERE 
                IDVerificacao = 20
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possível problema encontrado (' + CAST(@Quantidade AS VARCHAR(10)) + ' tentativas)' END),
            DSDetalhes = @Resultado
        WHERE 
            IDVerificacao = 100
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found (' + CAST(@Quantidade AS VARCHAR(10)) + ' attempts)' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Erro_Login_Senha_Incorreta>', 'Wrong_Username_Password>')
        WHERE 
            IDVerificacao = 100
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuario_SA_Habilitado>', 'SA_User_Enabled>')
        WHERE 
            IDVerificacao = 101
...
WHERE
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possível problema encontrado (' + CAST(@Quantidade AS VARCHAR(10)) + ' usuários)' END),
            DSDetalhes = @Resultado
        WHERE 
            IDVerificacao = 102
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found (' + CAST(@Quantidade AS VARCHAR(10)) + ' users)' END),
            DSDetalhes = REPLACE(REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Orfaos>', 'Orphaned_Users>'), '<Usuario ', '<User '), 'usuario=', 'user=')
        WHERE 
            IDVerificacao = 102
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Sem_Politica_de_Senha>', 'Users_Without_Password_Policy>'), '<Usuario ', '<User ')
        WHERE 
            IDVerificacao = 103
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Senha_Antiga>', 'Users_Old_Password>'), '<Usuario ', '<User ')
        WHERE 
            IDVerificacao = 104
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Senha_Fraca>', 'Users_Weak_Password>')
        WHERE 
            IDVerificacao = 105
...
WHERE
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found (' + CAST(@Quantidade AS VARCHAR(10)) + ' users)' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Sem_Permissao>', 'Users_Without_Permissions>'), '<Usuario ', '<User ')
        WHERE 
            IDVerificacao = 106
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN ISNULL(@Qt_Kerberos, 0) >= ISNULL(@Qt_NTLM, 0) THEN 'OK' Else 'Possível problema encontrado (' + CAST(ISNULL(@Qt_NTLM, 0) AS VARCHAR(10)) + ' conexões NTLM)' END),
            DSDetalhes = @Resultado
        WHERE 
            IDVerificacao = 107
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN ISNULL(@Qt_Kerberos, 0) >= ISNULL(@Qt_NTLM, 0) THEN 'OK' Else 'Possible issue found (' + CAST(ISNULL(@Qt_NTLM, 0) AS VARCHAR(10)) + ' NTLM connections)' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_AD_Sem_Kerberos>', 'Users_Without_Kerberos>'), '<Sessao ', '<Session ')
        WHERE 
            IDVerificacao = 107
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissao_View_Any_Database>', 'Users_View_Any_Database>'), '<Usuario ', '<User ')
        WHERE 
            IDVerificacao = 108
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissao_BUILTIN>', 'Users_BUILTIN>'), '<Usuario ', '<User ')
        WHERE 
            IDVerificacao = 109
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissao_Local>', 'Users_Local_Group>'), '<Usuario ', '<User ')
        WHERE 
            IDVerificacao = 110
 ...
 ---------------------------------------------------------------------------------------------------------------
     ...
     Set 
      DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
      DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissao_PUBLIC_role>', 'Permission_PUBLIC_role>'), '<Usuario ', '<User ')
     WHERE 
      IDVerificacao = 111
 ...
 ---------------------------------------------------------------------------------------------------------------
     ...
     Set 
      DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
      DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissao_PUBLIC_proxy>', 'Permission_PUBLIC_proxy>'), '<Usuario ', '<User ')
     WHERE	 
      IDVerificacao = 112
...
WHERE 
    ...
    Begin
        ...
        Begin
            ...
            UPDATE #Resultado
            Set 
                DSResultado = (CASE WHEN @Versao < 2008 THEN 'Não suportado' WHEN @Resultado IS NULL THEN 'OK' Else 'Possível problema encontrado' END),
                DSDetalhes = @Resultado
            WHERE 
                IDVerificacao = 300
        ...
        Begin
            ...
            UPDATE #Resultado
            Set 
                DSResultado = (CASE WHEN @Versao < 2008 THEN 'Não suportado' WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Databases_Sem_TDE>', 'Databases_Without_TDE>')
            WHERE 
                IDVerificacao = 300
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Databases_Sem_Backup>', 'Databases_Without_Backup>')
        WHERE 
            IDVerificacao = 301
    ...
    Begin
        ...
        Begin
            ...
            UPDATE #Resultado
            Set 
                DSResultado = (CASE WHEN @Versao <= 2008 THEN 'Not Supported' WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Backups_Sem_Criptografia>', 'Backups_Without_Encryption>')
            WHERE 
                IDVerificacao = 302
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Databases_Sem_Backup_de_LOG>', 'Backups_Without_Log_Backup>')
        WHERE 
            IDVerificacao = 303
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Extensao_Padrao_Arquivos_SQL>', 'SQL_Datafiles_Extension>')
        WHERE 
            IDVerificacao = 304
    ...
    Set @Resultado = (
        ...
        WHERE
            ...
            AND A.backup_start_date >= DATEADD(DAY, -7, @DtHoje )
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Extensao_Padrao_Backups_SQL>', 'SQL_Backup_Extension>')
        WHERE 
            IDVerificacao = 305
    ...
    Set @Resultado = (
        ...
        FROM
            #Informacoes_Backups A
        WHERE
            A.backup_start_date >= DATEADD(DAY, -7, @DtHoje )
            AND A.device_type = 2 -- Disco
            AND NOT EXISTS(SELECT NULL FROM #Informacoes_Backups WHERE [database_name] = A.[database_name] AND device_type <> 2 AND backup_start_date >= DATEADD(DAY, -7, @DtHoje ) )
        GROUP BY
            A.[database_name],
            A.logical_device_name,
            A.device_type,
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Armazenamento_Backups_SQL>', 'Backup_Device>')
        WHERE 
            IDVerificacao = 306
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissao_CONTROL_SERVER>', 'CONTROL_SERVER_Permission>')
        WHERE 
            IDVerificacao = 400
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Sysadmin_SecurityAdmin>', 'Sysadmin_SecurityAdmin_Users>')
        WHERE 
            IDVerificacao = 401
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Impersonate_Any_Login>', 'Users_Impersonate_Any_Login>')
        WHERE 
            IDVerificacao = 402
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Impersonate_Login>', 'Users_Impersonate_Login>')
        WHERE 
            IDVerificacao = 403
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Impersonate_Login_Sysadmin>', 'Users_Impersonate_Login_Sysadmin>')
        WHERE 
            IDVerificacao = 404
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_DB_Owners>', 'Users_DB_Owner>'), '<Usuario ', '<User '), 'usuario=', 'user=')
        WHERE 
            IDVerificacao = 405
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Impersonate_User>', 'Users_Impersonate>')
        WHERE 
            IDVerificacao = 406
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissoes_Public>', 'Public_Permissions>'), '<Objetos_Sistema', '<System_Objects'), '<Objetos_Usuario', 'User_Objects')
            WHERE 
                IDVerificacao = 407
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissoes_Guest>', 'Guest_Permissions>'), '<Objetos_Sistema ', '<System_Objects '), '<Objetos_Usuario ', 'User_Objects ')
            WHERE 
                IDVerificacao = 408
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Usuarios_Unsafe_External_Assembly>', 'Users_Unsafe_External_Assembly>'), '<Usuario ', '<User ')
        WHERE 
            IDVerificacao = 409
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Permissoes_Extended_Procedures>', 'Permissions_Extended_Procedures>'), '<Permissao ', '<Permission ')
        WHERE 
            IDVerificacao = 410
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Objetos_Com_IMPERSONATE>', 'Objects_With_Impersonate>'), '<Objeto ', '<Object ')
            WHERE 
                IDVerificacao = 500
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Objetos_Query_Dinamica>', 'Objects_With_Dynamic_Query>'), '<Objeto ', '<Object ')
            WHERE 
                IDVerificacao = 501
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Objetos_Utilizando_xp_cmdshell>', 'Objects_With_xp_cmdshell>'), '<Objeto ', '<Object ')
            WHERE 
                IDVerificacao = 502
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Objetos_Utilizando_OLE_Automation>', 'Objects_With_OLE_Automation>'), '<Objeto ', '<Object ')
            WHERE 
                IDVerificacao = 503
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Procedures_Executadas_Automaticamente>', 'Startup_Procedures>'), '<Objeto ', '<Object ')
        WHERE 
            IDVerificacao = 504
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Objetos_Utilizando_Grant>', 'Objects_With_Grants>'), '<Objeto ', '<Object ')
            WHERE 
                IDVerificacao = 505
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'LinkedServer_Usuario_Fixo>', 'LinkedServer_Fixed_User>')
        WHERE 
            IDVerificacao = 506
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Porta_Utilizada>', 'Used_Port>')
        WHERE 
            IDVerificacao = 600
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'SQL_Browser_Rodando_Apenas_1_Instancia>', 'SQL_Browser_1_Instance>')
            WHERE 
                IDVerificacao = 601
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_HideInstance>', 'Installation_HideInstance>')
            WHERE 
                IDVerificacao = 602
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_Usuario_Servico>', 'Installation__Service_Account>')
            WHERE 
                IDVerificacao = 603
    ...
    Begin
        ...
        Begin
            ...
            UPDATE #Resultado
            Set 
                DSResultado = (CASE WHEN @VersaoWindows_ProductName LIKE '% Server %' AND (@VersaoWindows_ProductName LIKE '% Server 2016%' OR @VersaoWindows_ProductName LIKE '% Server 2019%') THEN 'OK' Else 'Possível problema encontrado' END),
                DSDetalhes = @Resultado
            WHERE 
                IDVerificacao = 604
        ...
        Begin
            ...
            UPDATE #Resultado
            Set 
                DSResultado = (CASE WHEN @VersaoWindows_ProductName LIKE '% Server %' AND (@VersaoWindows_ProductName LIKE '% Server 2016%' OR @VersaoWindows_ProductName LIKE '% Server 2019%') THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_Versao_Windows>', 'Installation_Windows_Version>')
            WHERE 
                IDVerificacao = 604
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN CONVERT(INT, SERVERPROPERTY('ProductMajorVersion')) >= 11 THEN 'OK' Else 'Possível problema encontrado' END),
            DSDetalhes = @Resultado
        WHERE 
            IDVerificacao = 605
    ...
    Begin
        ...
        UPDATE #Resultado
        Set 
            DSResultado = (CASE WHEN CONVERT(INT, SERVERPROPERTY('ProductMajorVersion')) >= 11 THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_Versao_Nao_Suportada>', 'Installation_Not_Supported_Version>')
        WHERE 
            IDVerificacao = 605
    ...
    Begin TRY
        ...
        Begin
            ...
            Begin
                ...
                UPDATE #Resultado
                Set 
                    DSResultado = (CASE WHEN CONVERT(VARCHAR(100), SERVERPROPERTY('ProductVersion')) >= @Ultimo_Build THEN 'OK' Else 'Possível problema encontrado' END),
                    DSReferencia = @Url_Ultima_Versao_SQL,
                    DSDetalhes = @Resultado
                WHERE 
                    IDVerificacao = 606
            ...
            Begin
                ...
                UPDATE #Resultado
                Set 
                    DSResultado = (CASE WHEN CONVERT(VARCHAR(100), SERVERPROPERTY('ProductVersion')) >= @Ultimo_Build THEN 'OK' Else 'Possible issue found' END),
                    DSReferencia = @Url_Ultima_Versao_SQL,
                    DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_Atualizacoes_SQL>', 'Installation_SQL_Updates>')
                WHERE 
                    IDVerificacao = 606
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_Databases_Publicas>', 'Installation_Public_Databases>')
        WHERE 
            IDVerificacao = 607
    ...
    Begin
        ...
        Set 
            DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
            DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_Protocolos_de_Rede>', 'Installation_Network_Protocols>')
        WHERE 
            IDVerificacao = 608
    ...
    Begin
        ...
        Begin
            ...
            Set 
                DSResultado = (CASE WHEN @Resultado IS NULL THEN 'OK' Else 'Possible issue found' END),
                DSDetalhes = REPLACE(CAST(@Resultado AS VARCHAR(MAX)), 'Instalacao_Windows_Firewall>', 'Installation_Windows_Firewall>')
            WHERE 
                IDVerificacao = 609
    ...
    Begin
        ...
        SELECT 
            IDVerificacao AS [Code],
            DSCategoria AS [Category],
            DSTitulo AS [Title],
            DSResultado AS [Result],
            DSDescricao AS [How this can be an Issue],
            DSVerificacao AS [Technical explanation],
            DSSugestao AS [How to Fix],
            DSDetalhes AS [Result Details],
            CONVERT(XML, DSReferencia) AS [External Reference]
        FROM 
            #Resultado
    ...
    Begin
        ...
        SELECT 
            IDVerificacao AS [Código],
            DSCategoria AS [Categoria],
            DSTitulo AS [O que é verificado],
            DSResultado AS [Avaliação],
            DSDescricao AS [Descrição do Problema],
            DSVerificacao AS [Detalhamento da Verificação],
            DSSugestao AS [Sugestão de Correção],
            DSDetalhes AS [Resultados da Validação],
            CONVERT(XML, DSReferencia) AS [URL de Referência]
        FROM 
            #Resultado
